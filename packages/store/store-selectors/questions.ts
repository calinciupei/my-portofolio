...
export const getAllQuestions = (state: InitialState): Questions =>
  state.questions ? state.questions : [];

export const getQuestionsLength = (state: InitialState): number =>
...
export const getIntrovertAnswers = (state: InitialState): Answer[] =>
  state.answers.length
    ? state.answers.filter((answer) => answer.type === "INTROVERT")
    : [];

export const getExtrovertAnswers = (state: InitialState): Answer[] =>
  state.answers.length
    ? state.answers.filter((answer) => answer.type === "EXTROVERT")
    : [];

export const createQuestionSelector = () =>
  createSelector(
    [getQuestionTitle, getQuestionOptions],
    (title, questions): Question => ({ title, questions })
  );

export const createAllQuestionsSelector = () =>
  createSelector([getAllQuestions, getQuestionsLength], (questions, all): {
    questions: Questions;
    all: number;
  } => ({ questions, all }));

export const createAnswerSelector = () =>
  createSelector([getAnswerById], (answer): Answer[] => answer);

export const createAllAnswerSelector = () =>
  createSelector([getAnswers], (answers): Answers => answers);

export const createTestResults = () =>
  createSelector(
    [getIntrovertAnswers, getExtrovertAnswers],
